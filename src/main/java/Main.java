...
public class Main {
    ...
    public static void motorPID(){
        Thread t = new Thread(()->{
            ...
            double kp_x = 3.55; //PID constants X
            double ki_x = 0.005;
            double kd_x = 2.05;

            double kp_y = 3.55; //PID constants Y
            double ki_y = 0.005;
            double kd_y = 2.05;
            ...
            double throttleX = 300;
            double throttleY = 300;
            ...
            while(true) {
                Spi spi = new Spi();
                double elapsedTime = Variables.elapsedTime;
                ...
                if (errorX < -3 && errorX < 3) {
                    pid_i_x = pid_i_x+(ki_x*errorX);
                }
                if (errorY < -3 && errorY < 3) {
                    pid_i_y = pid_i_y+(ki_y*errorY);
                }
    ...
    public static void listenSensor(){
        Thread t = new Thread(()-> {
    ...
    public static int byteToInt(byte low, byte high) {
        int result;
        byte[] bytes = {high,low};
        result = ((bytes[0] << 8) & 0xff00) | (bytes[1] & 0x00ff);
        return result;
    }
}
